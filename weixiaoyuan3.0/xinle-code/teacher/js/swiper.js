...
(function($){
  $.fn.qqFace = function(options){
    ...
    $(this).click(function(e){
      var strFace, labFace;
      if($('#'+id).length<=0){
        strFace = '<div id="'+id+'" class="swiper-container qqFace">' +
          '<div class="swiper-wrapper"><div class="swiper-slide"><ul>';
        for(var i=1; i<=75; i++){
          labFace = '['+tip+i+']';
          strFace += '<li><img src="'+path+i+'.gif" onclick="$(\'#'+option.assign+'\').setCaret();$(\'#'+option.assign+'\').insertAtCaret(\'' + labFace + '\');" /></li>';
          if( i % 20 == 0 ) strFace += '<li onclick="$(\'#'+option.assign+'\').delFace();"><i class="icon iconfont">&#xe65d;</i> </li></ul></div><div class="swiper-slide"><ul>';
        }
        strFace += '<li onclick="$(\'#'+option.assign+'\').delFace();"><i class="iconfont icon-iconfontchachashanchu"></i> </li></ul></div></div><div class="swiper-pagination"></div></div>';
      }
      //$(this).parent().append(strFace);
      $('#'+show_id).html(strFace);
...
//    });
    $("body>*").on("click",function(event){
      if ($(event.target).parents("#qqfacebox").length == 0) {
        $('#'+id).hide();
        $('#'+id).remove();
        //$("#saytext").focus();
        delete(startRangs);
...
jQuery.fn.extend({
  ...
  insertAtCaret: function(textFeildValue){
    ...
    if(document.all && textObj.createTextRange && textObj.caretPos){
      ...
      if(typeof(startRangs) == "undefined"){
        startRangs = rangeStart;
        // alert(startRangs);
      }else if(rangeStart != startRangs){
        // alert("我是刚获取的变量"+rangeStart+"我是全局变量："+startRangs);
        rangeStart = startRangs;
      }
  ...
  delFace: function(event) {
    ...
    var textObjStar = textObj.value.substring(0,rangeStart);
    var textObjEnd = textObj.value.substring(rangeStart);
    var starStr = textObjStar.lastIndexOf("[em_");
    var endStr = textObjStar.lastIndexOf("]");
    var strLength = textObjStar.length;// body...  
    var tempStr = textObjStar.substring(0,starStr) + textObjStar.substring(endStr+1,strLength);
    textObj.value = tempStr+textObjEnd;// body...
